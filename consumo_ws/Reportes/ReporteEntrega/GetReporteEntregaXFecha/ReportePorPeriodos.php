...
where  RE.REPO_SERV_ID=" . $servicio_id . " and RE.REPO_TIRE_ID=" . $tipo_id . " and RE.REPO_PERIODO BETWEEN '" . $periodo_inicial_oracle . "' AND '" . $periodo_final_oracle . "'";
  ...
  */
  ...
  $objSheet->setCellValue('S1', '-------------');
  $objSheet->setCellValue('T1', 'CODDXPPAL');
  $objSheet->setCellValue('U1', 'ID_PROVEEDOR');
  $objSheet->setCellValue('V1', 'NOMBRE_PROVEEDOR');
  $objSheet->setCellValue('W1', 'COD_SERVICIO_TECNICO');
  $objSheet->setCellValue('X1', 'DESC_SERVICIO_TECNICO');
  $objSheet->setCellValue('Y1', 'PERIODO_CONSULTADO');
  $objSheet->setCellValue('Z1', 'ESTADO_CODTECENTREGADO');
  ...
  while (($row = oci_fetch_array($st_tire, OCI_BOTH)) != false) {
    ...
    $objSheet->setCellValue('S' . $i, '-------------');
    $objSheet->setCellValue('T' . $i, $row["CODDXPPAL"]);
    $objSheet->setCellValue('U' . $i, $row["ID_PROVEEDOR"]);
    $objSheet->setCellValue('V' . $i, $row["NOMBRE_PROVEEDOR"]);
    $objSheet->setCellValue('W' . $i, $row["COD_SERVICIO_TECNICO"]);
    $objSheet->setCellValue('X' . $i, $row["DESC_SERVICIO_TECNICO"]);
    $objSheet->setCellValue('Y' . $i, $row["REPO_PERIODO"]);
    $objSheet->setCellValue('Z' . $i, $reportar);
  }
