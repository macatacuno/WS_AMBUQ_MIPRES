...
if ($periodo_final < $periodo_inicial) {
 ...
 DECODE(RE.NOPRESCRIPCION,NULL,'NO EXISTE',RE.NOPRESCRIPCION)NOPRESCRIPCION,
  DECODE(RE.TIPOTEC, 'P','PROCEDIMIENTO', 'M','MEDICAMENTO', 'N','PRODUCTOS NUTRICIONALES', 'S','SERVICIOS COMPEMENTARIOS', 'D','DISPOSITIVOS MEDICOS',RE.TIPOTEC)TIPOTEC,
  ...
  DECODE(PR.DURACION_EN_MESES,NULL,'NO EXISTE',PR.DURACION_EN_MESES) TOTAL_ENTREGAS,
   DECODE(PR.CANTIDAD_TRATAMIENTO,NULL,'NO EXISTE',PR.CANTIDAD_TRATAMIENTO
  ||' '
  ||DESC_DURACION_TRATAMIENTO) DURACION_TRATAMIENTO,
  CASE
    WHEN RE.NOENTREGA<DURACION_EN_MESES THEN '0'
  WHEN RE.NOENTREGA=DURACION_EN_MESES THEN '1'
  WHEN RE.NOENTREGA>DURACION_EN_MESES
    THEN 'ERROR'
    ELSE 'ERROR'
  END ULTENTREGA,
  DECODE(RE.CANTTOTENTREGADA,NULL,'NO EXISTE',RE.CANTTOTENTREGADA)CANTTOTENTREGADA,
  DECODE(PR.ENTREGA_POR_MES_ESTIMADA,NULL,'NO EXISTE',PR.ENTREGA_POR_MES_ESTIMADA)ENTREGA_POR_MES_ESTIMADA,
  CASE
  WHEN TRANSLATE(RE.CANTTOTENTREGADA, 'T 0123456789', 'T') is not null THEN 'ERROR'/*Se valida si el dato es un numero o no*/
  WHEN RE.CANTTOTENTREGADA<ENTREGA_POR_MES_ESTIMADA THEN '0'
  WHEN RE.CANTTOTENTREGADA=ENTREGA_POR_MES_ESTIMADA THEN '1'
  WHEN RE.CANTTOTENTREGADA>ENTREGA_POR_MES_ESTIMADA
    THEN 'ERROR'
    ELSE 'ERROR'
  END ENTREGACOMPLETA,
...
FROM OASIS4.WEBSERV_REPORTE_ENTREGA re
LEFT JOIN OASIS4.WEBSERV_DIRECCIONAMIENTOS D ON D.ID=re.ID AND D.REPO_TIRE_ID=re.REPO_TIRE_ID
LEFT JOIN OASIS4.WEBSERV_PRES_LIST_PROV PROV ON PROV.NOIDPROV=D.NOIDPROV
LEFT JOIN OASIS4.WEBSERV_PRES_PRES PP 
  ON  TRIM(PP.NOPRESCRIPCION) =TRIM(RE.NOPRESCRIPCION)
LEFT JOIN  OASIS4.VIEW_WEBSERV_pres_info_direc PR 
