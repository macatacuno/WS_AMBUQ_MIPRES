...
if ($periodo_final < $periodo_inicial) {
  ...
  FROM OASIS4.WEBSERV_PRES_PRES pp
  LEFT JOIN OASIS4.WEBSERV_REF_PRE_AMB_ATE paa ON paa.CODIGO=pp.CODAMBATE
  LEFT JOIN OASIS4.WEBSERV_REF_PRE_ENF_UER EH ON EH.CODIGO=pp.CODENFHUERFANA
  LEFT JOIN OASIS4.WEBSERV_REF_PRE_CIE_10 CIE_CODDXPPAL ON CIE_CODDXPPAL.CODIGO=PP.CODDXPPAL
  LEFT JOIN OASIS4.WEBSERV_REF_PRE_CIE_10 CIE_CODDXREL1 ON CIE_CODDXREL1.CODIGO=PP.CODDXREL1
  LEFT JOIN OASIS4.WEBSERV_REF_PRE_CIE_10 CIE_CODDXREL2 ON CIE_CODDXREL2.CODIGO=PP.CODDXREL2
  LEFT JOIN OASIS4.WEBSERV_REF_PRE_TI_TR TT ON PP.TIPOTRANSC=TT.CODIGO
  LEFT JOIN OASIS4.WEBSERV_REF_PRE_EPS PE ON PP.CODEPS=PE.CODIGO
  ...
  LEFT JOIN OASIS4.client c on c.clientcode = PP.NROIDPACIENTE /*and c.TypeDocumentId=PP.TIPOIDPACIENTE*/ 
         AND c.clientid in (select max(c.CLIENTID)/*,c.CLIENTCODE,count(*)*/ cantidad from OASIS4.client c  group by c.clientcode)  
  LEFT join OASIS4.GeographicLocation  gl_muni  on c.CompanyId = gl_muni.CompanyId and c.GeographicLocationId = gl_muni.GeographicLocationId
  LEFT join OASIS4.GeographicLocation  gl_depa  on gl_muni.CompanyId = gl_depa.CompanyId 
...
order by REPO_PERIODO";
  ...
  $objXLS
    ->getActiveSheet()
    ->setTitle("PRESC GENERAL") // AQUI AGREGO EL NOMBRE A LA HOJA
    /*->getStyle('A3')
    ->getNumberFormat()
  ...
  $objSheet->setCellValue('AB1', 'PACCOVID19');

  //$objSheet->setCellValue('AD1', 'ENFHUERFANA');
  $objSheet->setCellValue('AC1', 'DESC_ENFHUERFANA');
  //$objSheet->setCellValue('AF1', 'CODENFHUERFANA');
  $objSheet->setCellValue('AD1', 'DESC_CODENFHUERFANA');
  // $objSheet->setCellValue('AH1', 'ENFHUERFANADX');
  $objSheet->setCellValue('AE1', 'DESC_ENFHUERFANADX');
  // $objSheet->setCellValue('AJ1', 'CODDXPPAL');
  $objSheet->setCellValue('AF1', 'DESC_CODDXPPAL');
  // $objSheet->setCellValue('AL1', 'CODDXREL1');
  $objSheet->setCellValue('AG1', 'DESC_CODDXREL1');
  //$objSheet->setCellValue('AN1', 'CODDXREL2');
  $objSheet->setCellValue('AH1', 'DESC_CODDXREL2');
  $objSheet->setCellValue('AI1', 'SOPNUTRICIONAL'); //
  //  $objSheet->setCellValue('AQ1', 'CODEPS'); //
  $objSheet->setCellValue('AJ1', 'DESC_CODEPS'); //
  $objSheet->setCellValue('AK1', 'TIPOIDMADREPACIENTE'); //
  $objSheet->setCellValue('AL1', 'NROIDMADREPACIENTE'); //
  // $objSheet->setCellValue('AU1', 'TIPOTRANSC');
  $objSheet->setCellValue('AM1', 'DESC_TIPOTRANSC');
  $objSheet->setCellValue('AN1', 'TIPOIDDONANTEVIVO');
  $objSheet->setCellValue('AO1', 'NROIDDONANTEVIVO');
  $objSheet->setCellValue('AP1', 'ESTPRES');
  ...
  while (($row = oci_fetch_array($st_tire, OCI_BOTH)) != false) {
    ...
    $objSheet->setCellValue('X' . $i, $MUNICIPIO);
    
    $DEPARTAMENTO = utf8_encode($row["DEPARTAMENTO"]);
    $objSheet->setCellValue('Y' . $i, $DEPARTAMENTO);
...
FROM OASIS4.WEBSERV_PRES_MEDI pm
LEFT JOIN OASIS4.WEBSERV_PRES_PRES       PP ON PM.ID_PRES=    PP.ID_PRES
LEFT JOIN OASIS4.WEBSERV_REF_PRE_TI_ME   TM ON PM.TIPOMED=    TM.CODIGO 
LEFT JOIN OASIS4.WEBSERV_REF_PRE_TI_PR   TP ON PM.TIPOPREST=  TP.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_FO_FA   FF ON PM.CODFF=      FF.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_VI_AD   VA ON PM.CODVA=      VA.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_UN_ME   UM ON PM.DOSISUM=    UM.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_TIEMPOS FA ON PM.CODFREADMON=FA.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_IN_ES   IE ON PM.INDESP=     IE.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_TIEMPOS DT ON PM.DURTRAT=    DT.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_UF_CT   UC ON PM.UFCANTTOTAL=UC.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_ES_JP   EJ ON PM.ESTJM=EJ.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_AMB_ATE PAA ON PAA.CODIGO=pp.CODAMBATE  LEFT JOIN OASIS4.client c on c.clientcode = PP.NROIDPACIENTE /*and c.TypeDocumentId=PP.TIPOIDPACIENTE*/ 
AND c.clientid in (select max(c.CLIENTID)/*,c.CLIENTCODE,count(*)*/ cantidad from OASIS4.client c  group by c.clientcode)
LEFT join OASIS4.GeographicLocation gl_muni  on c.CompanyId = gl_muni.CompanyId and c.GeographicLocationId = gl_muni.GeographicLocationId
LEFT join OASIS4.GeographicLocation gl_depa  on gl_muni.CompanyId = gl_depa.CompanyId 
               and gl_muni.Parent=gl_depa.GeographicLocationId and gl_muni.\"Level\">2  
...
*/
  ...
  FROM OASIS4.WEBSERV_PRES_INDI_UNIRS PIU
  LEFT JOIN OASIS4.WEBSERV_REF_PRE_IND_UNI FIU ON PIU.CODINDICACION=FIU.CODIGO
  LEFT JOIN OASIS4.WEBSERV_PRES_MEDI PM ON PIU.ID_MEDI=PM.ID_MEDI
  LEFT JOIN OASIS4.WEBSERV_PRES_PRES PP ON PM.ID_PRES=PP.ID_PRES
where  pp.REPO_SERV_ID=" . $servicio_id . " and pp.REPO_TIRE_ID=" . $tipo_id . " and pp.REPO_PERIODO BETWEEN '" . $periodo_inicial_oracle . "' AND '" . $periodo_final_oracle . "'";
  $st_tire = oci_parse($conn_oracle, $query);
  oci_execute($st_tire, OCI_DEFAULT);
...
FROM OASIS4.WEBSERV_PRES_PROC PPR
LEFT JOIN OASIS4.WEBSERV_PRES_PRES PP ON PP.ID_PRES=PPR.ID_PRES
LEFT JOIN OASIS4.WEBSERV_REF_PRE_TI_PR TP ON TP.CODIGO=PPR.TIPOPREST 
LEFT JOIN OASIS4.WEBSERV_REF_PRE_CUPS CPU ON CPU.CODIGO=PPR.PROPBSUTILIZADO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_CUPS CPD ON CPD.CODIGO=PPR.PROPBSDESCARTADO  
LEFT JOIN OASIS4.WEBSERV_REF_PRE_CUPS CP ON CP.CODIGO=PPR.CODCUPS  
LEFT JOIN OASIS4.WEBSERV_REF_PRE_TIEMPOS FU ON FU.CODIGO=PPR.CODFREUSO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_TIEMPOS DT ON DT.CODIGO=PPR.CODPERDURTRAT
LEFT JOIN OASIS4.WEBSERV_REF_PRE_ES_JP   EJ ON EJ.CODIGO=PPR.ESTJM
LEFT JOIN OASIS4.WEBSERV_REF_PRE_AMB_ATE paa ON paa.CODIGO=pp.CODAMBATE
LEFT JOIN OASIS4.client c on c.clientcode = PP.NROIDPACIENTE /*and c.TypeDocumentId=PP.TIPOIDPACIENTE*/ 
AND c.clientid in (select max(c.CLIENTID)/*,c.CLIENTCODE,count(*)*/ cantidad from OASIS4.client c  group by c.clientcode)
LEFT join OASIS4.GeographicLocation gl_muni  on c.CompanyId = gl_muni.CompanyId and c.GeographicLocationId = gl_muni.GeographicLocationId
LEFT join OASIS4.GeographicLocation gl_depa  on gl_muni.CompanyId = gl_depa.CompanyId 
  ...
  */
  ...
  $query = " 
    ...
    FROM OASIS4.WEBSERV_PRES_PROD_NUTR PPN
    LEFT JOIN OASIS4.WEBSERV_PRES_PRES PP ON PP.ID_PRES=PPN.ID_PRES
    LEFT JOIN OASIS4.WEBSERV_REF_PRE_TI_PR TP ON PPN.TIPOPREST=TP.CODIGO
    LEFT JOIN OASIS4.WEBSERV_REF_PRE_DIAG DXF ON PPN.DXENFHUER=DXF.CODIGO
    LEFT JOIN OASIS4.WEBSERV_REF_PRE_DIAG DXV ON PPN.DXVIH=DXV.CODIGO
    LEFT JOIN OASIS4.WEBSERV_REF_PRE_DIAG DXC ON PPN.DXCAPAL=DXC.CODIGO
    LEFT JOIN OASIS4.WEBSERV_REF_PRE_DIAG DXE ON PPN.DXENFRCEV=DXE.CODIGO
    LEFT JOIN OASIS4.WEBSERV_REF_PRE_DIAG DXD ON PPN.DXDESPRO=DXD.CODIGO
    LEFT JOIN OASIS4.WEBSERV_REF_PRE_TP_NU TN ON PPN.TIPPPRONUT=TN.CODIGO
    LEFT JOIN OASIS4.WEBSERV_REF_PRE_PR_NU PN ON PPN.DESCPRODNUTR=PN.CODIGO
    LEFT JOIN OASIS4.WEBSERV_REF_PRE_FP_NU FN ON PPN.CODFORMA=FN.CODIGO
    LEFT JOIN OASIS4.WEBSERV_REF_PRE_VA_PN VA ON PPN.CODVIAADMON=VA.CODIGO
    LEFT JOIN OASIS4.WEBSERV_REF_PRE_UN_ME UM ON PPN.DOSISUM=    UM.CODIGO
    LEFT JOIN OASIS4.WEBSERV_REF_PRE_TIEMPOS FA ON PPN.CODFREADMON=FA.CODIGO
    LEFT JOIN OASIS4.WEBSERV_REF_PRE_TIEMPOS DT ON PPN.DURTRAT=    DT.CODIGO
    LEFT JOIN OASIS4.WEBSERV_REF_PRE_FP_NU FCT ON PPN.UFCANTTOTAL=FCT.CODIGO
    LEFT JOIN OASIS4.WEBSERV_REF_PRE_ES_JP   EJ ON PPN.ESTJM=EJ.CODIGO
    LEFT JOIN OASIS4.WEBSERV_REF_PRE_AMB_ATE paa ON paa.CODIGO=pp.CODAMBATE
    LEFT JOIN OASIS4.client c on c.clientcode = PP.NROIDPACIENTE /*and c.TypeDocumentId=PP.TIPOIDPACIENTE*/ 
    AND c.clientid in (select max(c.CLIENTID)/*,c.CLIENTCODE,count(*)*/ cantidad from OASIS4.client c  group by c.clientcode)
               LEFT join OASIS4.GeographicLocation gl_muni  on c.CompanyId = gl_muni.CompanyId and c.GeographicLocationId = gl_muni.GeographicLocationId
...
FROM OASIS4.WEBSERV_PRES_SERV_COMP SC
LEFT JOIN OASIS4.WEBSERV_REF_PRE_TI_PR TP ON SC.TIPOPREST=  TP.CODIGO
LEFT JOIN OASIS4.WEBSERV_PRES_PRES PP ON PP.ID_PRES=SC.ID_PRES
LEFT JOIN OASIS4.WEBSERV_REF_PRE_SC_PR SP ON SC.CODSERCOMP=SP.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_TIEMPOS FU ON SC.CODFREUSO=FU.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_TIEMPOS PD ON SC.CODPERDURTRAT=PD.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_TI_TRANSPORTE TT ON SC.TIPOTRANS=TT.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_TD_AA TA ON SC.TIPOIDACOMALB=TA.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_PA_AA PA ON SC.PARENTACOMALB=PA.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_ES_JP EJ ON SC.ESTJM=EJ.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_AMB_ATE paa ON paa.CODIGO=pp.CODAMBATE
LEFT JOIN OASIS4.client c on c.clientcode = PP.NROIDPACIENTE /*and c.TypeDocumentId=PP.TIPOIDPACIENTE*/ 
AND c.clientid in (select max(c.CLIENTID)/*,c.CLIENTCODE,count(*)*/ cantidad from OASIS4.client c  group by c.clientcode)
           LEFT join OASIS4.GeographicLocation gl_muni  on c.CompanyId = gl_muni.CompanyId and c.GeographicLocationId = gl_muni.GeographicLocationId
...
where  pp.REPO_SERV_ID=" . $servicio_id . " and pp.REPO_TIRE_ID=" . $tipo_id . " and pp.REPO_PERIODO BETWEEN '" . $periodo_inicial_oracle . "' AND '" . $periodo_final_oracle . "'";
  ...
  $objSheet->setCellValue('O1', 'CANTTOTAL');
  $objSheet->setCellValue('P1', 'CANT');
  ...
  while (($row = oci_fetch_array($st_tire, OCI_BOTH)) != false) {
    ...
    $objSheet->setCellValue('O' . $i, $row["CANTTOTAL"]);
    $objSheet->setCellValue('P' . $i, $row["CANT"]);
...
FROM OASIS4.WEBSERV_PRES_DISP PD
LEFT JOIN OASIS4.WEBSERV_PRES_PRES PP ON PD.ID_PRES=PP.ID_PRES
LEFT JOIN OASIS4.WEBSERV_REF_PRE_TI_PR   TP ON PD.TIPOPREST=  TP.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_TD_ME TM ON PD.CODDISP=TM.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_TIEMPOS PDU ON PD.CODPERDURTRAT=PDU.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_ES_JP   EJ ON PD.ESTJM=EJ.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_AMB_ATE paa ON paa.CODIGO=pp.CODAMBATE 
LEFT JOIN OASIS4.client c on c.clientcode = PP.NROIDPACIENTE /*and c.TypeDocumentId=PP.TIPOIDPACIENTE*/ 
AND c.clientid in (select max(c.CLIENTID)/*,c.CLIENTCODE,count(*)*/ cantidad from OASIS4.client c  group by c.clientcode)
           LEFT join OASIS4.GeographicLocation gl_muni  on c.CompanyId = gl_muni.CompanyId and c.GeographicLocationId = gl_muni.GeographicLocationId
...
header("Content-Disposition: attachment;filename=Prescripciones " . $regimen . " " . $periodo_inicial . " - " . $periodo_final . ".xlsx");
