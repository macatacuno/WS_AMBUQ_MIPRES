...
FROM OASIS4.WEBSERV_JUNTA_PROFESIONAL JP 
LEFT JOIN OASIS4.WEBSERV_REF_JP_TI_TE  TT ON JP.TIPOTECNOLOGIA=TT.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_ES_JP EJ ON JP.ESTJM=EJ.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_JP_MODAL  MO ON JP.MODALIDAD=MO.CODIGO
LEFT JOIN OASIS4.WEBSERV_REF_PRE_TD_AA TA ON JP.TIPOIDPACIENTE=TA.CODIGO 
LEFT JOIN (SELECT B.ESTADO,B.TIDPODOCUMENTO,B.DOCUMENTO, B.DEPARTAMENTO, B.MUNICIPIO,B.NOM_MPIO,B.NOM_DPTO,B.MES 
           ...
           WHERE B.MES IN (SELECT MAX(MES) 
                           FROM ZZZ_BDUAHISSUB)) UB ON UB.TIDPODOCUMENTO=jp.TIPOIDPACIENTE AND UB.DOCUMENTO=jp.NROIDPACIENTE
